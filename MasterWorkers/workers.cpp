...
void worker( string tags[], string qos[], int socketworker_fd, Socket chancolas, int& tiempo){
    ...
    string enviartags="TAGS:";
    for(unsigned int i=0;i<5;i++){
        enviartags += tags[i]+SEPARADOR;
    }
    int send_bytes = chancolas.Send(socketworker_fd,enviartags);
    if(send_bytes == -1) {
        cerr << "Error al recibir datos: " + string(strerror(errno)) + "\n";
        // Cerramos los sockets
        chancolas.Close(socketworker_fd);
    ...
    string enviarqos="QOS:";
    for(unsigned int i=0;i<5;i++){
        enviarqos += qos[i]+SEPARADOR;
    }
    int send_bytes2 = chancolas.Send(socketworker_fd,enviarqos);
    if(send_bytes2 == -1) {
        cerr << "Error al recibir datos: " + string(strerror(errno)) + "\n";
        // Cerramos los sockets
        chancolas.Close(socketworker_fd);
...
int main(int argc, char *argv[]){
    ...
    for (int i=0; i<N_WORKERS; i++) {
        ...
        W[i] = thread(&worker,tags,qos,socketworker_fd[i],chancolas,ref(tiempo));
    }
