...
void conectar(Socket chanstreaming, string tweets[], string bloques[], int &error_code_streaming,Socket chancolas, int &error_code_colas){
  ...
  while(1){
    ...
    for(int j=0;j<25;j++){
      int rcv_bytes = chanstreaming.Recv(socketstreaming_fd,tweets[j],LONG_25TWEETS);
      if (rcv_bytes == -1) {
          cerr << "Error al recibir datos: " + string(strerror(errno)) + "\n";
          // Cerramos los sockets
          chanstreaming.Close(socketstreaming_fd);
      }
      cout << "Tweets recibidos: "<<tweets[j]<<endl;
    }
    ...
    for(unsigned int i=0;i<5;i++){
      bloques[i] = "";
        for(unsigned int j=0;j<5;j++){
            bloques[i]+=tweets[5*i+j]+SEPARADOR;
        }
    }
    ...
    //for(int j=0;j<5;j++){
      //if (bloques[j]!=""){
        for(int k=0;k<25;k++){
          ...
          cout << "Bloques enviados a gestor de colas: "<<tweets[k]<<endl;
        //}
