...
void servtags(controlColas& monitorTags,Socket socket_tags,int client_tags_fd){
    ...
    string solicitud = "";
    while(!end){
        recv_bytes = socket_tags.recv(client_taSgs_fd,solicitud,MESSAGE_SIZE); /*Nos tienen que enviar una solicitud de que quieren que les enviemos los datos*/
        if(recv_bytes == -1){
            cerr << "Error al recibir mensaje del Analizador QoS: " << strerror(errno) << endl;
            // Cerramos el socket
        ...
        if(solicitud == "END"){ /*Si la solicitud es END cerramos el canal, si es RECIBIR entonces les enviamos el dato que quieren*/
            ...
            send_bytes = socket_tags.Send(client_tags_fd, qos);
            if(send_bytes == -1){
...
int main(int argc,char* argv[]) {
    ...
    for(int i = 0;i<N_WORKER;i++){
        ...
        worker[i] = thread(&servworker,ref(monitorTareas),ref(monitorQos),ref(monitorTags),socket_worker,client_worker_fd); //El que le pasamos aqui es el client_worker_fd
