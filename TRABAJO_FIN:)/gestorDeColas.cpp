...
void servworker(ControlColas& monitorTareas,ControlColas& monitorQos,ControlColas& monitorTags,Socket socket_worker,int client_worker_fd){
    ...
    while(!end){
        ...
        if(solicitud == "END"){
            ...
            //cout << "Bloque enviado al worker: " << bloque << endl;
            if(send_bytes == -1){
            ...
            //cout << "Tags guardados en la cola: " << tags << endl;
            ...
            cout << "Qos guardados en la cola" << qos << endl;
        }
    }
}
...
void servtags(ControlColas& monitorTags,Socket socket_tags,int client_tags_fd){
    ...
    while(!end){
        ...
        if(recv_bytes == -1){
            cerr << "Error al recibir mensaje del Analizador Tags: " << strerror(errno) << endl;
            // Cerramos el socket
